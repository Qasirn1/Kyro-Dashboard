...
const createTracker = new Map();

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

module.exports = async (channel, config) => {
  try {
    ...
    if (!settings?.antiChannelCreate?.enabled) return;
 ...
 const entry =
  logs.entries.find((auditEntry) => {
    ...
    return targetId === channel.id && Math.abs(now - createdAt) < 15000;
  }) ||
  logs.entries.find((auditEntry) => {
    return auditEntry.target?.id === channel.id;
  });
    ...
    const limit = Math.max(
      1,
      Number(settings.antiChannelCreate?.limit) || 3
    );

    const trackerKey = `${guild.id}:channelCreate:${executorId}`;

    if (!createTracker.has(trackerKey)) {
      createTracker.set(trackerKey, []);
    }

    const timestamps = createTracker.get(trackerKey);
    timestamps.push(now);

    const filtered = timestamps.filter((ts) => now - ts < timeWindowMs);
    createTracker.set(trackerKey, filtered);
    ...
    if (member) {
      if (punishment === "kick") {
        if (member.kickable) {
          await member
            .kick("Kyro Anti-Nuke: Channel create threshold reached")
            .catch(() => {});
        ...
        if (member.bannable) {
          await member
            .ban({
              ...
              reason: "Kyro Anti-Nuke: Channel create threshold reached",
            })
            .catch(() => {});
          ...
          if (removableRoles.size > 0) {
            await member.roles
              .remove(
                ...
                "Kyro Anti-Nuke: stripping roles after channel create detection"
              )
              .catch(() => {});
          }
