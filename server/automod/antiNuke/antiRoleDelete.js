...
module.exports = async (role, config) => {
  try {
    ...
    if (!settings?.antiRoleDelete?.enabled) return;
  ...
  logs.entries.find((auditEntry) => {
    return auditEntry.target?.id === role.id;
  });
    ...
    const limit = Math.max(1, Number(settings.antiRoleDelete?.limit) || 3);

    const trackerKey = `${guild.id}:roleDelete:${executorId}`;

    if (!roleDeleteTracker.has(trackerKey)) {
      roleDeleteTracker.set(trackerKey, []);
    }

    const timestamps = roleDeleteTracker.get(trackerKey);
    timestamps.push(now);

    const filtered = timestamps.filter((ts) => now - ts < timeWindowMs);
    roleDeleteTracker.set(trackerKey, filtered);
    ...
    if (member) {
      if (punishment === "kick") {
        if (member.kickable) {
          await member
            .kick("Kyro Anti-Nuke: Role delete threshold reached")
            .catch(() => {});
        ...
        if (member.bannable) {
          await member
            .ban({
              ...
              reason: "Kyro Anti-Nuke: Role delete threshold reached",
            })
            .catch(() => {});
          ...
          if (removableRoles.size > 0) {
            await member.roles
              .remove(
                ...
                "Kyro Anti-Nuke: stripping roles after role delete detection"
              )
              .catch(() => {});
          }
